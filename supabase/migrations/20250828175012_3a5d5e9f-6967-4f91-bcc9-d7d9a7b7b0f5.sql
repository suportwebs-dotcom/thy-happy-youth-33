-- Primeiro, vamos verificar se as tabelas achievements já existem antes de inserir dados
DO $$ 
BEGIN
    -- Verificar se a tabela achievements possui uma coluna 'name'
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'achievements' AND column_name = 'name') THEN
        -- Se não existir, criar a estrutura necessária para achievements
...
-- Agora inserir os dados das conquistas se não existirem
INSERT INTO achievements (name, description, icon, category, requirement_type, requirement_value, points_reward)
SELECT * FROM (VALUES
-- Streak achievements
...
) AS new_achievements(name, description, icon, category, requirement_type, requirement_value, points_reward)
WHERE NOT EXISTS (
