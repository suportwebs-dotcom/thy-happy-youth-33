-- Enhanced security measures for subscribers table (corrected)
...
-- Add additional constraint to ensure user_id is never null (if not already set)
DO $$
BEGIN
  IF NOT EXISTS (
...
CREATE OR REPLACE VIEW public.subscriber_safe_view 
...
SELECT 
  ...
  created_at,
  updated_at,
  -- Mask email to show only first 3 chars for authorized users only
  CASE 
    WHEN public.validate_subscriber_access(user_id) THEN 
      CONCAT(LEFT(email, 3), '***@', RIGHT(SPLIT_PART(email, '@', 2), 3))
    ELSE '***@***.***'
  END as email_masked,
...
RETURNS VOID AS $$
BEGIN
  -- Log security events for monitoring (admin can query pg_stat_statements)
